...
describe('orders (e2e)', () => {
  ...
  it('publishing marks the order PUBLISHED and it appears in the branch orders', async () => {
    ...
    const published = await request(app.getHttpServer())
      .post(`/orders/${order.body.id}/publish`)
      .set(auth(fixtures.adminToken))
      .expect(201);
  ...
  it('excludes empty orders from the branch list and caps it at RECENT_ORDER_LIMIT', async () => {
    ...
    expect(list.body.some((o: { id: string }) => o.id === empty.body.id)).toBe(
      false,
    );
  }, 60000);
  ...
  it('stops a staff user touching an order for a provider they lack access to', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .send({
        branchId: fixtures.branchId,
        providerId: fixtures.providerIds[2],
      })
      .expect(403);
